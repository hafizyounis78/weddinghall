...
public function get_booking_notification()
    {
		...
		$after_date=date('Y-m-d', strtotime('+3 days'));
		$myquery = "SELECT 		payments. * , wedding_booking. * , customer. * , wedding_hall. *,booking_status_tb.*
					FROM   		wedding_booking
					LEFT JOIN 	payments ON wedding_booking.booking_code = payments.booking_code
					AND 		payment_status <>4, customer, wedding_hall,booking_status_tb
